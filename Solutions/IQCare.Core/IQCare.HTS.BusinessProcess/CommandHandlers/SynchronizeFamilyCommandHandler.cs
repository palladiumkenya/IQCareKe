                    ...
                    for (int i = 0; i < request.FAMILY.Count; i++)
                    {
                        ...
                        if (indexClientIdentifiers.Count > 0)
                        {
                            ...
                            for (int j = 0; j < request.FAMILY[i].ENCOUNTER.TRACING.Count; j++)
                            {
                                ...
                                DateTime reminderDate = DateTime.ParseExact(request.FAMILY[i].ENCOUNTER.TRACING[j].REMINDER_DATE, "yyyyMMdd", null);
                                DateTime tracingBookingDate = DateTime.ParseExact(request.FAMILY[i].ENCOUNTER.TRACING[j].BOOKING_DATE, "yyyyMMdd", null);
