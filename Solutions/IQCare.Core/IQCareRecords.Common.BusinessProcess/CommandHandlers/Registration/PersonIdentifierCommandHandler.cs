...

            {
                ...
                if(request.PersonId> 0)
                {
                    PersonIdentifier pidm = new PersonIdentifier();
                     pidm =await Task.Run(()=>rs.GetCurrentPersonIdentifier(request.IdentifierId,request.PersonId));

                    if(pidm !=null)
                    {
                        pidm.DeleteFlag = true;
                       var pdm = await Task.Run(() => rs.UpdatePersonIdentifier(pidm));
                       var  finalupdate = await Task.Run(() => rs.addPersonIdentifiers(request.PersonId, request.IdentifierId, request.IdentifierValue, request.UserId));
                        if(finalupdate !=null)
                        {
                            if(finalupdate.Id > 0)
                            {
                    ...
                    {
                       var finalIdent = await Task.Run(() => rs.addPersonIdentifiers(request.PersonId, request.IdentifierId, request.IdentifierValue, request.UserId));
                        if (finalIdent != null)
                        {
                            if (finalIdent.Id > 0)
                            {
