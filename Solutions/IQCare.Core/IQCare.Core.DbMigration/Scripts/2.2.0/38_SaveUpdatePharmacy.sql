...
/****** Object:  StoredProcedure [dbo].[sp_SaveUpdatePharmacy_GreenCard]    Script Date: 09/09/2019 15:27:35 ******/
SET ANSI_NULLS ON
GO
...
Begin               
	...
	IF EXISTS(select 1 from ord_PatientPharmacyOrder where PatientMasterVisitId = @PatientMasterVisitID and ProgID=@TreatmentProgram) 
	--IF EXISTS(select 1 from ord_PatientPharmacyOrder where ptn_pharmacy_pk = @Ptn_Pharmacy_Pk) 
	BEGIN
		set @Ptn_Pharmacy_Pk = (select top 1 ptn_pharmacy_pk from ord_PatientPharmacyOrder where patientmasterVisitID = @PatientMasterVisitID and ProgID =@TreatmentProgram);
		IF @TreatmentPlan = 0 BEGIN (select TOP 1 @Regimenline = RegimenLine, @TreatmentProgram = [ProgID] from ord_PatientPharmacyOrder where patientmasterVisitID = @PatientMasterVisitID and ProgID=@TreatmentProgram); END;
		...
		Where patientmasterVisitID = @PatientMasterVisitID and ProgID=@TreatmentProgram;
		...
		Select ptn_pharmacy_pk = @ptn_pharmacy_pk;
	END
	ELSE
	BEGIN
		...
		Select ptn_pharmacy_pk = @ptn_pharmacy;
