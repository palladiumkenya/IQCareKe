...
ALTER VIEW [dbo].[Maternity Register]
AS
SELECT       distinct  a.Ptn_Pk, b.Id, b.FacilityId, h.IdentifierValue AS [Admission Number], d.VisitDate AS [Date of Admission], a.FirstName, a.MiddleName, a.LastName, a.VillageName, a.Phone, a.Landmark, a.DOB, a.Sex, a.MaritalStatus, 
                         ...
                         delivery.PlacentaComplete,delivery.BloodLosscapacity, delivery.MotherCondition AS [Condition after Delivery], delivery. [DeliveryComplicationsExperienced], DeliveryConductedBy,
						 babyGender=
									(SELECT        TOP 1 ItemName
									  FROM            [dbo].[LookupItemView]
									  WHERE        ItemId =baby.Sex), baby.BirthWeight, DeliveryOutcome=
									(SELECT        TOP 1 ItemName
									  FROM            [dbo].[LookupItemView]
									  WHERE        ItemId =baby.DeliveryOutcome), baby.BreastFedWithinHr, 
                         baby.TeoGiven, baby.BirthDeformity, APGAR1min, APGAR5min, APGAR10min, 
						 CASE WHEN j.[On ARV Before 1st ANC Visit]='Yes' then 'KP'
							  WHEN HAARTANC.[Started HAART in ANC]='Yes' then 'P'
							  WHEN HAARTMAT.[Started HAART MAT]='Yes' then 'N'
							  else 'NA' end as ANC
						 ,'' [HIV testing],
						 RPRLab.TestResults1 AS TreatedForSyphillis,HAARTMAT.[Started HAART MAT],
						 HIVTest.OneKitId,HIVTest.OneLotNumber,HIVTest.OneExpiryDate,HIVTest.FinalTestOneResult,
						 HIVTest.twokitid,HIVTest.twolotnumber,HIVTest.twoexpirydate,HIVTest.FinalTestTwoResult, z.FinalResult FinalResult, 
						 k.Weight, k.Height, k.BPSystolic, k.BPDiastolic, k.Muac, baby.BirthNotificationNumber, 
						 PatOutcome.DateDischarged [DateDischarged], patOutcome.OutcomeStatus OutcomeStatus, 
						 partnerTesting.[PartnerTested],partnerTesting.[PartnerHIVResult],
						 Refferals.ReferredFrom, Refferals.ReferredTo,[MaternalDeathAuditDate], notes.ClinicalNotes
FROM            dbo.mst_Patient AS a INNER JOIN
						...
						Left join 
						(Select DeliveredBabyBirthInformationId,max(APGAR1min)APGAR1min,max(APGAR1min)APGAR5min,Max(APGAR1min)APGAR10min from (
								select a.Id,a.[DeliveredBabyBirthInformationId],APGAR1min=
									(SELECT        TOP 1 Score
									  FROM            [dbo].[LookupItemView]
									  WHERE        ItemId =a.[ApgarScoreId] and itemname='Apgar Score 1 min'),
									  APGAR5min=
									(SELECT        TOP 1 Score
									  FROM            [dbo].[LookupItemView]
									  WHERE        ItemId =a.[ApgarScoreId] and itemname='Apgar Score 5 min'),APGAR10min =
									(SELECT        TOP 1 Score
									  FROM            [dbo].[LookupItemView]
									  WHERE        ItemId =a.[ApgarScoreId] and itemname='Apgar Score 10 min')
								from [dbo].[DeliveredBabyApgarScore] a 
								inner join [dbo].[DeliveredBabyBirthInformation]b on a.[DeliveredBabyBirthInformationId]=b.[Id]
								)a group by DeliveredBabyBirthInformationId)apgar on apgar.DeliveredBabyBirthInformationId=baby.id	
						LEFT JOIN (SELECT b.Patientid,[PatientMasterVisitID]
										  ,[DateDischarged]
										  ,[OutcomeStatus]=
											(SELECT        TOP 1 ItemName
											  FROM            [dbo].[LookupItemView]
									  ...
									  where a.DeleteFlag=0) PatOutcome  on PatOutcome.PatientId=b.Id and PatOutcome.PatientMasterVisitId=d.id
						LEFT OUTER JOIN 
						------RPR
									...
									where testname like '%RPR%'and hasresult=1)RPRLab on RPRLab.PatientId=b.Id and RPRLab.OrderedByDate=d.[VisitDate] 
