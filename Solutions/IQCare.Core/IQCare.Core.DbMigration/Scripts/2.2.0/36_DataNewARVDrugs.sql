IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NewDrugList]') AND type in (N'U')) 
BEGIN
...
INSERT INTO NewDrugList(DrugName,Abbreviation,strength) VALUES('Abacavir/Lamivudine-600mg/300mg','ABC/3TC','600mg/300mg');
...
INSERT INTO NewDrugList(DrugName,Abbreviation,strength) VALUES('Darunavir/Ritonavir-800mg/100mg','DRV/r','800mg/100mg');

Declare @ItemId int=0,@genericId int,@count int,@counter int=0,@maxId int=0,@minId int=0;
Declare @drugName varchar(100),@abbreviation varchar(20),@strength varchar(10);
...
VALUES (0,''+@drugName+'',(SELECT top 1 isnull(ItemTypeID,378) FROM Mst_ItemSubType WHERE SubTypeName='ARV Medication'),0,1,GETDATE(),''+@abbreviation+'');
