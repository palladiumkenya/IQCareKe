...
SELECT DISTINCT pt.Id
	...
	,pe.CareEnded
	,CASE(SELECT COUNT(Id) from PatientCareending D where D.DeleteFlag = 0 AND D.PatientId = pt.Id)
	WHEN 0 THEN 'Active'
	ELSE (SELECT TOP 1 ItemName FROM LookupItemView WHERE MasterName = 'CareEnded' AND ItemId = ptC.ExitReason) END AS PatientStatus
	,ptC.ExitReason
	,pt.DateOfBirth
