...
Select	MP.Ptn_Pk
	...
	,	MP.DeleteFlag
	,	Status
	,	PatientEnrollmentID = (SELECT top 1 PI.IdentifierValue FROM PatientIdentifier PI WHERE IdentifierTypeId = 1 AND PI.PatientId = P.Id)
	,CardSerialNumber
From mst_Patient MP
LEFT JOIN Patient P ON P.ptn_pk = MP.Ptn_Pk
Where (MP.DeleteFlag = 0 Or MP.DeleteFlag Is Null) AND P.DeleteFlag = 0
