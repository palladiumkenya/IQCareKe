...
select distinct ISNULL(ROW_NUMBER() OVER(ORDER BY po.ptn_pharmacy_pk ASC), -1) AS RowID,
po.PatientId,po.PatientMasterVisitId,po.VisitID,v.VisitDate,
...
po.orderstatus,
(select top 1 lt.DisplayName from LookupItem lt where  lt.Id= po.PharmacyPeriodTaken) as [PeriodTakenText] , 
artv.Regimen,
artv.RegimenId,
artv.RegimenLineId,
...
left join ord_Visit v on v.Visit_Id=po.VisitID
...
left join (SELECT        D.Item_PK AS Drug_pk, D.ItemCode AS DrugID, D.ItemTypeID, D.ItemName AS DrugName, D.DeleteFlag, D.CreatedBy AS UserID, D.CreateDate, D.UpdateDate, 
...
left join mst_Decode dc on dc.ID=po.ProgID
