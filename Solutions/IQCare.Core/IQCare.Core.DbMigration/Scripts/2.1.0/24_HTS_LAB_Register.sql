...
AS
SELECT DISTINCT ISNULL(ROW_NUMBER() OVER (ORDER BY HE.Id ASC), - 1) AS RowID, P.Id PatientID, p.Ptn_pk AS PatientPK, CONVERT(varchar(50), decryptbykey(Per.firstname)) + ' ' + CONVERT(varchar(50), 
...
Gender =
    ...
    (SELECT        TOP 1 ItemName
      FROM            [dbo].[LookupItemView]
      WHERE        ItemId = he.TestingStrategy), TestEntryPoint =
    (SELECT        TOP 1 ItemName
      FROM            [dbo].[LookupItemView]
...
FROM            [dbo].[PatientEncounter] PE INNER JOIN
		...
		(SELECT DISTINCT b.PatientId, max(d.UserName)UserName FROM  dbo.Patient AS a INNER JOIN dbo.PatientEncounter AS b ON a.Id = b.PatientId INNER JOIN
			dbo.HtsEncounter AS c ON a.PersonId = c.PersonId INNER JOIN dbo.mst_User AS d ON b.CreatedBy = d.UserID group by b.PatientId) UN on un.PatientId=pe.PatientId inner join
        [dbo].[HtsEncounterResult] HER ON HtsEncounterId = HE.Id LEFT JOIN
		(SELECT DISTINCT b.PatientId, max(lv.ItemName) AS TBScreening FROM dbo.Patient AS a INNER JOIN
			...
			WHERE lv.MasterName LIKE '%TbScreening%' group by b.PatientId) screen on screen.patientid=pe.patientid left join
        [PatientMaritalStatus] ms ON ms.personid = p.personid LEFT JOIN
            ...
            (SELECT DISTINCT 
				...
				left join  [dbo].[PatientEncounter] pe on pe.id=e.PatientEncounterID  inner join 
				lookupitemview c on c.itemid=pe.EncounterTypeId left OUTER JOIN
				...
				inner join lookupitemview b on b.itemid=t.KitId inner join  [dbo].[PatientEncounter] pe on pe.id=e.PatientEncounterID  
						inner join lookupitemview c on c.itemid=pe.EncounterTypeId WHERE  e.encountertype = 1 and t.testround =1 and c.ItemName='Hts-encounter') one ON one.personid = e.PersonId FULL OUTER JOIN
				(SELECT distinct htsencounterid, b.ItemName kitid, kitlotnumber, expirydate, PersonId, l.ItemName AS outcome
				FROM  [Testing] t inner join  [HtsEncounter] e on t.HtsEncounterId=e.id inner join  [LookupItemView] l on l.itemid=t.Outcome
			inner join lookupitemview b on b.itemid=t.KitId inner join  [dbo].[PatientEncounter] pe on pe.id=e.PatientEncounterID  
		inner join lookupitemview c on c.itemid=pe.EncounterTypeId WHERE   t.testround =2 and c.ItemName='Hts-encounter') two ON two.personid = e.PersonId where c.ItemName='Hts-encounter'  ) kits ON kits.personid = p.personid
