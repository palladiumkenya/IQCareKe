    ...
    loadTBAssessment(): void {
            ...
            (result) => {
                ...
                */
                ...
                for (let i = 0; i < result.length; i++) {
                    // On TB drugs
                    const onAntiTbDrugs = result[i].onAntiTbDrugs ? 'Yes' : 'No';
        ...
        this.heiservice.getPatientIcfAction(this.patientId).subscribe(
            (result) => {
                ...
                for (let i = 0; i < result.length; i++) {
                    // ICF Action
                    this.TbAssessmentFormGroup.get('sputumSmear').setValue(result[i].sputumSmear);
                    this.TbAssessmentFormGroup.get('geneXpert').setValue(result[i].geneXpert);
                    this.TbAssessmentFormGroup.get('chestXray').setValue(result[i].chestXray);
                    this.TbAssessmentFormGroup.get('invitationContacts').setValue(result[i].invitationOfContacts);
                }
            },
            (error) => {
                console.log(error);
