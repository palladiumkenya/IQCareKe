...
export class HeiHivtestingComponent implements OnInit {
    ...
    ngOnInit() {
        const { hivTestType, testResults, YesNo } = this.heiHivTestingOptions[0];
            ...
            (result) => {
                for (let i = 0; i < result.length; i++) {
                    if (result[i].key == 'PCR') {
                        ...
                        this.heiservice.getLabTestPametersByLabTestId(this.pcrLabTest.id).subscribe(
                            (res) => {
                        ...
                        this.heiservice.getLabTestPametersByLabTestId(this.viralLoadLabTest.id).subscribe(
                            (res) => {
                        ...
                        this.heiservice.getLabTestPametersByLabTestId(this.antibodyLabTest.id).subscribe(
                            (res) => {
            ...
            (res) => {
                for (let i = 0; i < res.length; i++) {
                    ...
                    if (savedHeiLabTests.length > 0) {
                        testType = this.hivTestType.filter(obj => obj.itemId == savedHeiLabTests[0]['heiLabTestTypeId']);
                    } else {
                        testType = this.hivTestType.filter(obj => obj.itemName.includes(res[i].labTestName));
                    }
    ...
    public completeLabOrder(hivTestingTableData) {
        ...
        const dialogRef = this.dialog.open(HeiCompletelaborderComponent, dialogConfig);
        dialogRef.afterClosed().subscribe(
            data => {
                if (!data) {
                    return;
                ...
                if (
                    ...
                    || hivTestingTableData.testtype['itemName'] == 'Repeat confirmatory PCR (for +ve)'
                    || hivTestingTableData.testtype['itemName'] == 'Confirmatory PCR (for  +ve)'
                ) {
                ...
                } else if (hivTestingTableData.testtype['itemName']['itemName'] == 'Baseline Viral Load (for +ve)') {
                    completeLabOrderCommand.LabTestResults.push({
                ...
                } else if (hivTestingTableData.testtype['itemName']['itemName'] == 'Final Antibody') {
                    completeLabOrderCommand.LabTestResults.push({
                ...
                this.heiservice.saveCompleteHeiLabOrder(completeLabOrderCommand).subscribe(
                    (res) => {
                        this.hiv_testing_history_data = [];
                        this.snotifyService.success('Successfully Update HIV tests ', 'HEI', this.notificationService.getConfig());
                        this.loadPatientCompletedTestTypes();
                    },
                    (error) => {
                        this.snotifyService.error('An error occured while updating HEI hiv tests ', 'HEI',
                            this.notificationService.getConfig());
                    }
                );
            }
