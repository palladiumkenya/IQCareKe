...
{
    public partial class PayBillExempt : System.Web.UI.UserControl, IPayment
    {
        #region variables
        /// <summary>
        /// The valid
        ...
        public string ClientScript
        {
            get
            {
                string script = "function SelectReason() { var ctrl = $('# ";
                script += ddlExemption.ClientID + "');var selectedReason = ctrl.val();var selectedText = ctrl.text();alert(selectedReason + ' ' + selectedText);";
                script += "if (selectedReason === \"\") {$(\"#" + buttonCompute.ClientID + "\").attr(\"disabled\", \"disabled\"); }";
                script += " else {$(\"#" + buttonCompute.ClientID + "\").removeAttr(\"disabled\"); }}";
                return script;
            }
        }
        /// <summary>
        ...
        public void Compute()
        {
            ...
            isValidEntry = validateDropDownList(ddlExemption);
            ...
            BillPaymentInfo payObject = new BillPaymentInfo()
            {
                ...
                Narrative = ddlExemption.SelectedItem.Text,
        ...
        {
          //  ScriptManager.RegisterClientScriptBlock(ddlExemption, this.GetType(), this.UniqueID, this.ClientScript, false);
        ...
        /// <summary>
        /// Populates the exemption reason.
        /// </summary>
        void PopulateExemptionReason()
        {
            if (ddlExemption.Items.Count == 0)
            {
                IBilling BManager = (IBilling)ObjectFactory.CreateInstance("BusinessProcess.Billing.BBilling, BusinessProcess.Billing");
                ddlExemption.Items.Clear();
                try
                {
                    List<KeyValuePair<int, string>> reasons = BManager.GetExemptionReasons("");
                    if (null != reasons)
                    {
                        reasons.ForEach(r =>
                        {
                            ddlExemption.Items.Add(new ListItem(r.Value, r.Key.ToString()));
                        });
                    }
                }
                catch { }
                ddlExemption.Items.Insert(0, new ListItem("Select", ""));
            }
        }
        /// <summary>
