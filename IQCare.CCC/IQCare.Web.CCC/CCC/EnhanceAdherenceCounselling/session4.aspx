            ...
            success: function (response) {
                ...
                $.each(JSON.parse(response.d), function (index, value) {
                    ...
                    var resp = JSON.parse(response.d);
                     var ret = resp[index].ClinicalNotes;
                    var res = value.ClinicalNotes;
                      inputnotes =this.ClinicalNotes;
                        //response[i].ClinicalNotes;
                    //this.NotesCategoryId;

                    if ($("#session4tb" + resp[index].NotesCategoryId).length > 0) {
                        
                        $("#session4tb" +  resp[index].NotesCategoryId).val(inputnotes);
                    }
            });
                var s4fd = $('input[type="text"].s4followupdateinput').val();
                gets4AppointmentId(s4fd);
            },
            error: function (response) {
