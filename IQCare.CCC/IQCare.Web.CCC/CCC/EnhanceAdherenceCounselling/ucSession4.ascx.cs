...
{
    public partial class ucSession4 : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                getAdherenceCtrls();
                getScoreCtrls();
                getSession4Data(PatientId, PatientMasterVisitId);
            }
        }
        ...
        protected void getSession4Data(int patientId, int patientMasterVisitId)
        {
            ...
            if (notesArray != null)
            {
                if (notesArray.Any())
                {
                    foreach (var value in notesArray)
                    {
                        ...
                        TextBox ntb = (TextBox)FindControl("session4tb" + value.NotesCategoryId.ToString());
                        if (ntb != null)
                        {
                            ntb.Text = value.ClinicalNotes;
                        }
                        if (LookupLogic.GetLookupItemId("Session4FollowupDate") == value.NotesCategoryId.ToString())
                        {
            ...
            if (screeningArray != null)
            {
                foreach (var value in screeningArray)
                {
                    ...
                    RadioButtonList rbl = (RadioButtonList)FindControl("session4rb" + value.ScreeningCategoryId.ToString());
                    if (rbl != null)
                    {
