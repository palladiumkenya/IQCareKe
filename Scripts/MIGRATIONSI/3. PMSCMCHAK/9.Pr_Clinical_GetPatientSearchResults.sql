...
BEGIN
	...
	Declare @Query nvarchar(max), 
			...
			@IdentifierSQL varchar(2000),
			@StatusStr VARCHAR(5200),
			@ByModule varchar(1000), @ByStatus varchar(1200);
			
	Declare @SymKey nvarchar(400)	;	 
	Select	@Identifiers = '',@ByModule='',@ByStatus=' Status =  Null ,',@StatusStr = ' And (@Status Is Null Or P.[Status] = @status)',@IdentifierSQL='PatientEnrollmentId Identifier';
...
Begin
	Select @ByModule= ' Left Outer Join (Select	P.Ptn_pk,P.ModuleId,P.StartDate EnrollmentDate,	Case CT.CareEnded When 1 Then ''Care Ended'' When 0 Then ''Active (Restarted)''  Else ''Active'' End CareStatus,		
		Case CT.CareEnded When 1 Then CT.PatientExitReasonName Else Null End  CareEndReason,	Isnull(CT.EnrollmentIndex,1)EnrollmentIndex,CASE CT.CareEnded WHEN 1 THEN 1 ELSE 0 END PatientCareEndStatus From dbo.Lnk_PatientProgramStart As P
...
And (@FacilityID Is Null Or P.LocationID=@FacilityID)' + @StatusStr +@Identifiers + ') P '+ @RuleFilter  ;
