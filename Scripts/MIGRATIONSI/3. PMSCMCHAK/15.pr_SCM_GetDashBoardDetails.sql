...
BEGIN
EXEC ('ALTER PROCEDURE [dbo].[pr_SCM_GetDashBoardDetails] 
@storeid int = 0
AS
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[pr_SCM_GetDashBoardDetails]') AND type in (N'P', N'PC'))
BEGIN
EXEC ('Create PROCEDURE [dbo].[pr_SCM_GetDashBoardDetails] 
@storeid int = 0
AS
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets from
