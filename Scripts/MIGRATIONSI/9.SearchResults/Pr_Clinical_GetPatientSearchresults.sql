...
/****** Object:  StoredProcedure [dbo].[Pr_Clinical_GetPatientSearchresults]    Script Date: 12/19/2018 1:47:05 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	Declare @Query nvarchar(max), 
			...
			@ByModule varchar(5000), @ByStatus varchar(5200);
...
Begin
	...
	if(@IdentifierName Is Null) Begin
		...
		+ Convert(varchar,@enrollmentid) + ''' or P.IQNumber=''' + Convert(varchar,@enrollmentid) +''')'*/
			Select @Identifiers = ' AND ( ' + Replace(@SS, ',', ' like ''%' + Convert(varchar,@enrollmentid) + '%'' or ') + ' like ''%' 
		+ Convert(varchar,@enrollmentid) + '%'' or P.IQNumber=''' + Convert(varchar,@enrollmentid) +''')'
