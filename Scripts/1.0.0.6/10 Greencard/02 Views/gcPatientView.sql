...
SELECT DISTINCT pt.Id
	,pt.PersonId
	,pt.ptn_pk
	,isnull((SELECT TOP 1 v.IdentifierValue FROM patientIdentifier v WHERE v.PatientId=pt.Id AND v.IdentifierTypeId IN(SELECT top 1 Id FROM identifiers z WHERE z.Code='CCCNumber' )), (
	...
	,CASE 
		WHEN (SELECT top 1 i.IdentifierValue FROM patientIdentifier i WHERE i.PatientId=pt.Id AND i.IdentifierTypeId IN(SELECT top 1 f.Id FROM Identifiers f WHERE f.Code='CCCNumber') AND pt.Id  NOT IN(SELECT d.PatientId FROM PatientCareending d WHERE d.ExitReason IN(SELECT top 1 Id FROM LookupItem WHERE Name='Death') OR d.ExitReason NOT IN(SELECT top 1 k.PatientId FROM PatientCareending k WHERE k.DeleteFlag=0))) IS NOT NULL THEN 'Active'
		WHEN (SELECT top 1 i.IdentifierValue FROM PatientIdentifier i WHERE i.PatientId=pt.Id AND i.IdentifierTypeId IN(SELECT top 1 f.Id FROM Identifiers f WHERE f.Code='CCCNumber') AND  pt.Id   IN(SELECT TOP 1 e.PatientId from PatientCareending e WHERE e.PatientId=pt.Id AND e.DeleteFlag=0))IS NOT NULL THEN (SELECT TOP 1 ItemName
								FROM LookupItemView
								WHERE MasterName = 'CareEnded'
									AND ItemId = ptC.ExitReason)
		-- WHEN (SELECT top 1 i.IdentifierValue FROM PatientIdentifier i WHERE i.PatientId=pt.Id AND i.IdentifierTypeId IN(SELECT top 1 f.Id FROM Identifiers f WHERE f.Code='CCCNumber') AND pt.Id  IN(SELECT e.PatientId from PatientCareending e WHERE e.ExitReason IN(SELECT top 1 Id FROM LookupItem WHERE Name='Death')))IS NOT NULL THEN 'Death'
		ELSE
		   'Not Enrolled'
	...
	--,ISNULL(CAST((
	--			CASE pe.CareEnded
	--				WHEN (SELECT top 1 i.IdentifierValue FROM patientIdentifier i WHERE i.PatientId=pt.Id AND pt.Id NOT IN(SELECT top 1 d.PatientId FROM PatientCareending d WHERE d.PatientId=pt.Id)) 
	--					THEN 'Active'
	--				WHEN 1
	--					THEN (
