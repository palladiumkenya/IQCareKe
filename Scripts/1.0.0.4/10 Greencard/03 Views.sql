...
SELECT 
	...
	ISNULL((SELECT top 1 id FROM LookupItem WHERE Name IN(CASE R.RegimenType
			...
			WHEN '3TC/EFV/TDF'	THEN CASE  WHEN R.age>=15 THEN 'AF2A' ELSE 'CF2A' END --'TDF + 3TC + NVP'
	...
	ISNULL((
		CASE R.RegimenType
			...
			WHEN '3TC/EFV/TDF'	THEN CASE WHEN R.age>= 15 THEN (SELECT top 1 Name FROM lookupitem WHERE Name='AF2A')ELSE (SELECT top 1 Name FROM lookupitem WHERE Name='AF2A')  END+'(TDF + 3TC + NVP)'
